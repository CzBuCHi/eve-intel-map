        ...
        public static void EnsureDatatabaseExists() {
           ...
           IEnumerable<string> q = from o in c.ConnectionString.Split(';')
            ...
            using (_Connection = new SQLiteConnection(c.ConnectionString)) {
                ...
                CreateTable_Races();
                CreateTable_ShipTypes();
                CreateTable_ShipNames();
                ...
                InsertData("eve_races.sql");
                InsertData("eve_ship_types.sql");
                InsertData("eve_ship_names.sql");
...
CREATE TABLE [eve_map_solarsystem_jumps] (
    ...
    FOREIGN KEY(to_region_id) REFERENCES eve_map_regions(region_id)
        ...
        private static void CreateTable_Races() {
            using (SQLiteCommand command = new SQLiteCommand(@"
...
CREATE TABLE [eve_ship_types] (
    ...
    [ship_type_name] TEXT NOT NULL
        ...
        private static void CreateTable_ShipNames() {
            using (SQLiteCommand command = new SQLiteCommand(@"
CREATE TABLE [eve_ship_names] (
    [ship_name] TEXT NOT NULL PRIMARY KEY,
    [ship_type_id] INTEGER NOT NULL,
    [tech_level] INTEGER NOT NULL,
    [race_id] INTEGER NOT NULL,

    FOREIGN KEY(ship_type_id) REFERENCES eve_ship_types(ship_type_id),
    FOREIGN KEY(race_id) REFERENCES eve_races(race_id)
)", _Connection)) {
                command.ExecuteNonQuery();
            }
        }
